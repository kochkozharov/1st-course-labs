...
#define PRINT_LIST_ITERATOR(printf_str, type, it) \
    if(listIteratorGet(&it)) printf(printf_str, *(const type *)listIteratorGet(&it)); else perror("");
...
const void *listFront(const List *list);

const void *listBack(const List *list);
