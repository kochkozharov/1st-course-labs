...
size_t stackCapacity(const Stack* const stack) { return stack->capacity; }

void stackClear(Stack* const stack) { stack->depth = 0; }
...
void stackDestroy(Stack* const stack) { free(stack->data); }

bool stackIsEmpty(const Stack* const stack) { return stack->depth == 0; }
...
int stackPushBack(Stack* const stack, const StackElem value) {
    ...
    if (stack->capacity == stack->depth) {
        ...
        StackElem* const data =
            realloc(stack->data, capacity * sizeof(StackElem));
...
int stackResize(Stack* const stack, const size_t newSize,
                const StackElem value) {
    ...
    if (newSize <= stack->capacity) {
        while (stack->depth < newSize) stack->data[stack->depth++] = value;
        return 0;
    }
...
size_t stackDepth(const Stack* const stack) { return stack->depth; }
